...
BEGIN
  ...
  -- Multiplier access checks (league-scoped)
  IF p_multiplier IN (3, 4, 5) THEN
    -- Check unlock exists and is unconsumed in this league
    IF NOT EXISTS (
      SELECT 1 FROM multiplier_unlocks
      WHERE user_id = p_user_id AND league_id = p_league_id
        AND multiplier = p_multiplier::TEXT AND is_consumed = false
    ) THEN
      RAISE EXCEPTION '%sx not unlocked in this league', p_multiplier;
    END IF;

    -- Check cooldown in this league
    SELECT matches_remaining INTO v_cooldown_rem
    FROM cooldowns
    WHERE user_id = p_user_id AND league_id = p_league_id
      AND multiplier = p_multiplier::TEXT;

    IF v_cooldown_rem IS NOT NULL AND v_cooldown_rem > 0 THEN
      RAISE EXCEPTION '%sx on cooldown — %s matches remaining', p_multiplier, v_cooldown_rem;
    END IF;
  END IF;

  -- Get league membership (credits)
  SELECT * INTO v_member FROM league_members
  ...
  -- Max bet: global config, capped by league setting
  v_max_bet := (get_config_value('max_bet_amount'))::NUMERIC;
  ...
  IF p_bet_amount > v_max_bet THEN
    RAISE EXCEPTION 'Bet exceeds max allowed: ₹%', v_max_bet;
  END IF;

  -- Late-bet penalty
  IF now() > v_match.bet_deadline THEN
    v_penalty_pct := (get_config_value('late_bet_penalty_pct'))::NUMERIC;
  END IF;

  v_locked_amount := CEIL(p_bet_amount * (1 + v_penalty_pct / 100.0));
  v_potential_win := FLOOR(p_bet_amount * p_multiplier);
